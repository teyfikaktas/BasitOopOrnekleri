...
public class TypingSystem extends ToDoMechanism {
    ...
    @Override
    public void notcomplatedıtem() { // TODO: 6.08.2021
        if (toDoMaterials.size() <= 0) {
            System.out.println("The to-do list appears empty. You can type \"create new todo\" to add a to-do list. ");
            StartingWhile();
            return;
        }
        for (ToDoMaterial toDoMaterial : toDoMaterials) {
            if (!toDoMaterial.getIs_Completed()) {
                System.out.println();
                System.out.print(toDoMaterial.getTitle() + "         ");
                System.out.print(toDoMaterial.getDescription() + "         ");
                System.out.print(toDoMaterial.getRemaindate() + "         ");
                System.out.print(LevelStyle(toDoMaterial.getLevel()) + "         ");
                System.out.print(MainClass.TranslateString(toDoMaterial.getIs_Completed()) + "            ");
                System.out.println();
            } else {
    ...
    public int AddItem(int index) {
        ...
        ToDoMaterial toDoMaterial = new ToDoMaterial(title,description,FormatterDateTime.Formatter(),Level);
        toDoMaterials.add(index, toDoMaterial);
        return 0;
    }
    @Override
    public int UpdateItem() {
        ...
        if (toDoMaterials.size() > 0) {
            ...
            } else if (UpdateItem.equals("description")) {
            ...
            } else if (UpdateItem.equals("level")) {
    ...
    public static void StartingWhile() {
        ...
        while (true) {
            ...
            if (Exist == 2) {
                TypingSystem typingSystem = new TypingSystem();
                typingSystem.UpdateItem();

            }
            if (Exist == 3) {
                TypingSystem typingSystem = new TypingSystem();
                typingSystem.DeleteItem();
            }
            if (Exist == 4) {
                TypingSystem typingSystem = new TypingSystem();
    ...
    public static void Settings() {
        ...
        if (s.equals("delete all todo items")) {
